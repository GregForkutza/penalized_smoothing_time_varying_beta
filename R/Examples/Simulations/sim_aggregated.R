...
source("R/Functions/construct_simulator/construct_simulator_sim_agg.R")
source("R/Functions/update_likelihood/update_likelihood_expressions_sim_agg.R")
source("R/Functions/calibrate_multiple_smoothers/calibrate_multiple_smoothers_sim_agg.R")
source("R/Functions/generate_simulated_data.R")
source("R/Functions/extract_simulation_results.R")
source("R/Functions/plotting/generate_comparative_plots_sim_agg.R")
...
# Model parameters   
  ...
  sim_sd = 500 # Add noise to simulated data
  N = 4000000
...
# Aggregate simulated incidence 
  # Define number of observations to aggregate over. 
  step_length = 7 
  # Add noise to aggregated data
  sim_data_results <- generate_simulated_data(simulator = simulator_initial, sim_sd = sim_sd, step_length = step_length)
  wk_inc_obs_noise <- sim_data_results$P_noise
...
calibrated_smoothers <- calibrate_multiple_smoothers( smooth_list, smooth_parms_fit, 
                                                      compartmental_parms_fit, model_parms, sir_spec, step_length,
                                                      eq=TRUE, wk_inc_obs_noise, aggregate = TRUE)

# Combine results into a list 
return_list <- list(
  initial_results = results_initial,
  calibrated_simulator = calibrated_smoothers,
  incidence = wk_inc_obs_noise
)
...
# Create plots 
  ...
  plots <- generate_comparative_plots(return_list, aggregate = TRUE)
  
  # Extract the plots from plots
  plot_incidence_gp <- plots$cr_gp_plots$plot_incidence
  plot_beta_gp <- plots$cr_gp_plots$plot_beta
  plot_R_t_gp <- plots$cr_gp_plots$plot_R_t
  
  plot_incidence_tp <- plots$cr_tp_plots$plot_incidence
  plot_beta_tp <-  plots$cr_tp_plots$plot_beta
  plot_R_t_tp <- plots$cr_tp_plots$plot_R_t
  
  plot_incidence_cr <- plots$cr_cr_plots$plot_incidence
  plot_beta_cr <- plots$cr_cr_plots$plot_beta
  plot_R_t_cr <- plots$cr_cr_plots$plot_R_t
  
  plot_incidence_bs <- plots$cr_bs_plots$plot_incidence
  plot_beta_bs <- plots$cr_bs_plots$plot_beta
  plot_R_t_bs <- plots$cr_bs_plots$plot_R_t
...
compute_mse_table(simulator_list = calibrated_smoothers, incidence = wk_inc_obs_noise, aggregate = TRUE)
