...
create_plots <- function(results, observed_vector, smooth_type, simulator, Date, aggregate) {
  ...
  p1 <- ggplot(data_combined_incidence, aes(x = Date)) +
    ...
    scale_x_date(breaks = c(first_date, last_date), labels = c(format(first_date, "%b"), format(last_date, "%b"))) +
    theme_minimal() 
  ...
  p3 <- ggplot(data_combined_R_t, aes(x = Date)) +
    ...
    p1 <- ggplot(data_combined_incidence, aes(x = Date)) +
      ...
      labs(x = NULL, y = NULL) +  # Remove axis labels
      theme_minimal() +
      scale_x_date(date_labels = "%b '%y", date_breaks = "12 month") # Show month and last two digits of year
...
generate_comparative_plots <- function(simulators, aggregate = FALSE, conf.levels = c(0.5, 0.95), data_name, Date, observed_vector) {
  ...
  for (smooth_type in calibrated_model_types) {
    ...
    results_fitted <- extract_simulation_results(simulator = simulators[[smooth_type]], conf.levels = c(0.5, 0.95), aggregate = aggregate)
    plots <- create_plots(results = results_fitted, observed_vector = observed_vector, smooth_type, simulators[[smooth_type]], Date, aggregate= aggregate)
...
extract_smooth_types <- function(all_comparative_plots) {
  ...
  # Extract the smooth types from the names
  smooth_types <- unique(sub(".*_(\\D{2})_plots.*", "\\1", all_names))
...
all_plots <- function(all_comparative_plots, output_dir = NULL, file_name = "comparative_plot", knots_vector, x_label = "Day") {
  ...
  plot_identifiers <- c("plot_incidence", "plot_beta", "plot_R_t")
