...
create_plots <- function(results, results_true, observed_vector, smooth_type, simulator) {
  ...
  data_combined_beta <- data.frame(
    ...
    Beta_hat_true = exp(results_true$value_theta),
  ...
  data_combined_R_t <- data.frame(
    ...
    R_t_true = exp(results_true$value_R_t),
...
generate_comparative_plots <- function(simulators, aggregate = FALSE, conf.levels = c(0.5, 0.95), knots_vector) {
  ...
  initial_model_type <- names(simulators$simulator_initial[1])
  calibrated_model_types <- names(simulators$calibrated_simulator)
  sim_initial <- simulators$simulator_initial[[initial_model_type]]

  results <- extract_simulation_results(simulator = sim_initial, conf.levels = conf.levels, aggregate = aggregate)
  
  for (smooth_type in calibrated_model_types) {
    results_fitted <- extract_simulation_results(simulator = simulators$calibrated_simulator[[smooth_type]], conf.levels = c(0.5, 0.95), aggregate = FALSE)
    ...
    plot_list_name <- paste(initial_model_type, smooth_type, "plots", sep = "_")
...
extract_smooth_types <- function(all_comparative_plots) {
  ...
  smooth_types <- unique(sub(".*_initial_(\\w{2})_plots.*", "\\1", all_names))
  return(smooth_types)
}
